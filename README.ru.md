...
Выгрузка Codex накопительная. Найденные новые промты объединяются с ранее выгруженными и дедуплицируются; уже сохранённые промты остаются, даже если исходная JSONL-сессия позже удалена. Структурированное состояние для append хранится в скрытом файле `codex/.prompts-state.json`. Не удаляйте его, если не хотите пересобрать историю Codex только из доступных сейчас сессий. Для существующих установок без state-файла один раз выполняется миграция из `codex/all-prompts.txt`. Файлы отдельных рабочих папок и `codex/all-prompts.txt` заново собираются в хронологическом порядке из накопленного состояния.
...
Выгрузка DeepSeek накопительная. Найденные новые промты объединяются с ранее выгруженными и дедуплицируются; уже сохранённые промты остаются, даже если содержащий их архив позже удалён. Структурированное состояние для append хранится в скрытом файле `deepseek/.prompts-state.json`. Не удаляйте его, если не хотите пересобрать историю DeepSeek только из доступных сейчас источников. Для существующих установок без state-файла один раз выполняется миграция из `deepseek/all-prompts.txt`.
...
В этом файле накопленные истории Codex и DeepSeek объединены в единый хронологический поток. Для каждого промта указываются тип `LLM` (`codex` или `deepseek`), ID сессии, timestamp, текст и рабочая папка, если источник содержит такую информацию.
